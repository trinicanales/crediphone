...
### [REACT-301] React error "Too many re-renders" en páginas con reparaciones
**Severidad:** CRÍTICA
**Detectado:** sesión previa (2026-04)
**Estado:** ✅ RESUELTO — commit `8530a69` (2026-04-04)
**Síntoma:** Crash completo de página en `/dashboard/reparaciones` y `/dashboard/tecnico`.
Error: *"Too many re-renders. React limits the number of renders to prevent an infinite loop."*

**Causa raíz 1 — `ModalWhatsAppEstado.tsx`:**
La función `handleClose()` se llamaba directamente durante la fase de render:
```tsx
// ❌ ANTES (bug):
if (!mensajeBase) {
  handleClose();   // ← setState durante render → React error #301
  return null;
}
```
`handleClose()` llama `setIniciado(false)`, `setMensajeEditado("")` y `onClose()` (setState del padre).
Esto ocurría en CADA render de `OrdenCard` porque `nuevoEstadoWA` inicia en `"recibido"`,
estado que no tiene plantilla de WhatsApp → `mensajeBase = ""` → se disparaba el bloque.

**Solución aplicada:**
```tsx
// ✅ DESPUÉS (correcto):
useEffect(() => {
  if (isOpen && !mensajeBase) { onClose(); }  // auto-close en efecto, no en render
}, [isOpen, mensajeBase, onClose]);

useEffect(() => {
  if (!isOpen) { setIniciado(false); setMensajeEditado(""); }  // reset en efecto
}, [isOpen]);

if (!isOpen || !mensajeBase) return null;  // solo return, sin setState
```

**Causa raíz 2 — `OrdenDrawer.tsx`:**
`TabResumen`, `TabDiagnostico`, `TabPresupuesto` eran funciones de componente React
definidas **dentro** del body de `OrdenDrawer`. React detecta referencia nueva en cada
render → desmonta y remonta el subtab completo en cada actualización.
```tsx
// ❌ ANTES: función dentro del componente usada como JSX
function TabResumen() { ... }
return <TabResumen />;  // nueva referencia → desmonta/remonta

// ✅ DESPUÉS: función con minúscula, llamada directa
function tabResumen() { ... }
return tabResumen();    // inlineado en el árbol del padre, sin desmontaje
```

**Regla permanente:** Ver notas 14 y 15 en CLAUDE.md.
...
### Resumen
- Limpieza de errores ESLint (24 errores `react/no-unescaped-entities` → `&quot;`) en 10 archivos
- Commit consolidado con 33 archivos pendientes: `ed84d4f` ("fix: consolida bug fixes multi-tenant + ESLint")
- Resolución de problemas de git en VM virtiofs (index.lock, config corruption) con pipeline Python plumbing
- Verificación de repo: `crediphone/crediphone` ya estaba transferido a `trinicanales/crediphone` (redirect 301)

### GitHub — Estado final
- **Repo activo:** `https://github.com/trinicanales/crediphone` (personal, sin org)
- **Org `crediphone`:** Eliminada por expiración de trial Enterprise — no hay riesgo de pérdida de datos
- **Commits pusheados:** `74292d1..ed84d4f` en `origin/master`
- **GitHub Actions secrets — 11 configurados:**

| Secret | Estado |
|---|---|
| CLOUDFLARE_ACCOUNT_ID | ✅ Ya existía |
| CLOUDFLARE_API_TOKEN | ✅ Ya existía |
| NEXT_PUBLIC_SUPABASE_ANON_KEY | ✅ Ya existía |
| NEXT_PUBLIC_SUPABASE_URL | ✅ Ya existía |
| SUPABASE_SERVICE_ROLE_KEY | ✅ Agregado 2026-04-04 |
| NEXT_PUBLIC_VAPID_PUBLIC_KEY | ✅ Agregado 2026-04-04 |
| VAPID_PUBLIC_KEY | ✅ Agregado 2026-04-04 |
| VAPID_PRIVATE_KEY | ✅ Agregado 2026-04-04 |
| PAYJOY_API_KEY | ✅ Agregado 2026-04-04 |
| PAYJOY_WEBHOOK_SECRET | ✅ Agregado 2026-04-04 |
| INTERNAL_API_SECRET | ✅ Agregado 2026-04-04 |

### Token de deploy en .git/config
El `.git/config` usa token clásico `crediphone-deploy-push-v2` (scope: repo + workflow).
Si expira, generar nuevo en: `https://github.com/settings/tokens`

### Próximo paso
Para desplegar a Cloudflare Workers: hacer un push o merge a `master` dispara el workflow `.github/workflows/deploy.yml` automáticamente en GitHub Actions.
